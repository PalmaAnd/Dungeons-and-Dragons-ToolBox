    ...
    const parseCustomRoll = (formula: string): RollResult | null => {
        ...
        matches.forEach(match => {
            ...
            if (sides === 100) {
                ...
                const d100Results = Array.from({ length: numDice }, () => rollD100());
                                ...
                                {die === "d20" && (
                                    <div className="flex gap-1">
                                        <Button 
                                            size="sm" 
                                            variant="secondary"
                                            onClick={() => handleAdvantageRoll(die)}
                                        ...
                                        <Button 
                                            size="sm" 
                                            variant="secondary"
                                            onClick={() => handleDisadvantageRoll(die)}
                                            className="flex-1 text-xs"
                                        >
                                            DIS
                                        </Button>
                    ...
                    {rolls.length === 0 ? (
                        ...
                        <div className="space-y-3 max-h-96 overflow-y-auto">
                            {rolls.map((roll, index) => (
                                <div key={roll.id} className="border rounded-lg p-3 space-y-2">
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center gap-2">
                                            <Badge variant="outline">{roll.formula}</Badge>
                                    ...
                                    <div className="space-y-2">
                                        {roll.details.dice.map((diceGroup, diceIndex) => (
                                            <div key={diceIndex} className="text-sm">
                                                <div className="flex items-center gap-2 flex-wrap">
                                                    <span className="font-medium">{diceGroup.type}:</span>
                                                    {diceGroup.rolls.length > 1 ? (
                                                        <>
                                                            <div className="flex gap-1">
                                                                {diceGroup.rolls.map((rollValue, rollIndex) => (
                                                                    <Badge 
                                                                        key={rollIndex} 
                                                                        variant="outline"
                                                                        className={`
                                                                            ${roll.details.advantage && rollValue === Math.max(...diceGroup.rolls) ? 'bg-green-500' : ''}
                                                                            ${roll.details.disadvantage && rollValue === Math.min(...diceGroup.rolls) ? 'bg-red-600' : ''}
                                                                            ${roll.details.advantage && rollValue !== Math.max(...diceGroup.rolls) ? 'opacity-50 line-through' : ''}
                                                                            ${roll.details.disadvantage && rollValue !== Math.min(...diceGroup.rolls) ? 'opacity-50 line-through' : ''}
                                                                        `}
                                                                    >
                                                                        {rollValue}
                                                                    </Badge>
                                                            ...
                                                            {!roll.details.advantage && !roll.details.disadvantage && (
                                                                <>
                                                                    <span>=</span>
                                                                    <Badge variant="secondary">{diceGroup.total}</Badge>
                                                                </>
                                                            )}
                                                        </>
                                                    ) : (
                                                        <Badge variant="secondary">{diceGroup.rolls[0]}</Badge>
                                                    )}
                                                </div>
                                                
                                                {/* D100 Special Display */}
                                                {roll.details.d100Details && diceGroup.type.includes('d100') && (
                                                    <div className="ml-4 text-xs text-muted-foreground">
                                                        Tens die: {roll.details.d100Details.tens === 0 ? '00' : roll.details.d100Details.tens * 10}, 
                                                        Ones die: {roll.details.d100Details.ones}
                                                    </div>
                                                )}
                                            </div>
                                        ))}
                                        ...
                                        {roll.details.modifier && (
                                            <div className="text-sm">
                                                ...
                                                <Badge variant="outline" className="ml-2">
                                                    {roll.details.modifier > 0 ? '+' : ''}{roll.details.modifier}
                                                </Badge>
                                            </div>
                                        )}
                                        
                                        {/* Final Total for Complex Rolls */}
                                        {(roll.details.dice.length > 1 || roll.details.modifier) && (
                                            <div className="text-sm border-t pt-2">
                                                <span className="font-medium">Total: </span>
                                                <Badge variant="default">{roll.total}</Badge>
                                            </div>
                                        )}
                                    </div>
                                </div>
