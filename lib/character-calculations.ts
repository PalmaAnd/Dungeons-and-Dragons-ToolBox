...
export const calculateRacialBonuses = (
    ...
    Object.entries(raceInfo.abilityScoreIncrease).forEach(([ability, bonus]) => {
        if (ability === "all") {
            ...
            modifiedAbilities[ability] = (modifiedAbilities[ability] || 0) + bonus;
        }
    });
    ...
    if (subrace && raceInfo.subraces) {
        ...
        if (subraceInfo) {
            Object.entries(subraceInfo.abilityScoreIncrease).forEach(([ability, bonus]) => {
                modifiedAbilities[ability] = (modifiedAbilities[ability] || 0) + bonus;
            });
        }
    }
...
export const calculateSpellSlots = (level: number, className: string): Record<string, number> => {
    const spellSlotProgression: Record<string, Record<number, Record<string, number>>> = {
        "Wizard": {
            1: { "1st": 2 },
            2: { "1st": 3 },
            3: { "1st": 4, "2nd": 2 },
...
export const getClassFeaturesByLevel = (level: number, className: string, subclass?: string): string[] => {
...
export const calculateArmorClass = (
    ...
    const armorItem = enhancedData.equipment.armor.find(a => a.name === armor);
    ...
    if (armorItem) {
        if (armorItem.type === "Light") {
            ...
            baseAC = parseInt(armorItem.armorClass.split("+")[0]) + Math.min(dexModifier, 2);
...
export const getAvailableSpells = (level: number, className: string, race: string, subrace: string, enhancedData: any): Record<string, any[]> => {
    ...
    if (spellData.classSpells[className]) {
        const classSpellData = spellData.classSpells[className];
...
export const getDefaultSpells = (level: number, className: string, race: string, subrace: string, enhancedData: any): Record<string, string[]> => {
    const spellData = enhancedData.spells;
    ...
    if (spellData.classSpells[className]) {
        const classSpellInfo = spellData.classSpells[className].level1;
        
        // For some classes, add recommended starting spells
        if (className === "Wizard" && level >= 1) {
            // Wizards typically start with basic utility spells
            if (defaultSpells.cantrips.length === 0) {
                defaultSpells.cantrips.push("Mage Hand", "Prestidigitation", "Fire Bolt");
            }
            if (defaultSpells.level1.length === 0) {
                defaultSpells.level1.push("Magic Missile", "Shield", "Detect Magic", "Burning Hands");
            }
        } else if (className === "Cleric" && level >= 1) {
            if (defaultSpells.cantrips.length === 0) {
                defaultSpells.cantrips.push("Sacred Flame", "Guidance", "Light");
            }
            // Clerics prepare spells, so we don't add level 1 spells by default
        } else if (className === "Sorcerer" && level >= 1) {
            if (defaultSpells.cantrips.length === 0) {
                defaultSpells.cantrips.push("Fire Bolt", "Mage Hand", "Prestidigitation", "Ray of Frost");
            }
            if (defaultSpells.level1.length === 0) {
            ...
            if (defaultSpells.cantrips.length === 0) {
                defaultSpells.cantrips.push("Vicious Mockery", "Minor Illusion");
            }
            if (defaultSpells.level1.length === 0) {
                defaultSpells.level1.push("Healing Word", "Thunderwave", "Cure Wounds", "Detect Magic");
            }
